...
public class PlanetTextureGen : MonoBehaviour
{
    ...
    private void Generate()
    {
        ...
        Color[] rawpixels = new Color[_horizontalResolution * VerticalResolution];
        Color[] mappedpixels = new Color[_horizontalResolution * VerticalResolution];

        Texture2D hMap = new Texture2D(_horizontalResolution, VerticalResolution, TextureFormat.RGB24, false);
        Texture2D colorMap = new Texture2D(_horizontalResolution, VerticalResolution, TextureFormat.RGB24, false);
        ...
        colorMap.SetPixels(mappedpixels);
        colorMap.Apply();

        hMap.SetPixels(rawpixels);
        hMap.Apply();

        GetComponent<Renderer>().material.SetTexture("_DiffuseTex", colorMap);
        GetComponent<Renderer>().material.SetTexture("_SpecularTex", hMap);
    ...
    private void GenerateEarthlike(object arg)
    {
        ...
        for (int q = starPos; q < qargs.memPerThread + starPos; q++)
        {
            ...
            qargs.rawPx[q] = new Color(landSample, landSample, landSample);

            if (ColorMapEnabled)
                targetColor = MapLandColor(landSample);
            else
                targetColor = qargs.rawPx[q];
