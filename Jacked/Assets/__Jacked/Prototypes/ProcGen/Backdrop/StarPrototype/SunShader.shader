...
Shader "Custom/SunShader" {
    Properties {
        ...
        [MaterialToggle] _UseTexture ("Use Texture", Float ) = NaN
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
...
////// Emissive:
                ...
                float3 emissive = ((pow(1.0-max(0,dot(normalDirection, viewDirection)),_iOR)*_FresnellColor.rgb)+lerp( lerp(_Color1.rgb,_Color2.rgb,lerp(0,0.5,pow(noise( (i.uv0*_NoiseUV) , _NoiseComplexity , _Seed ),_Contrast))), _Diffuse_var.rgb, _UseTexture ));
